...
export const AddressList: React.FC<AddressListProps> = ({ addresses, onEdit, onDelete, saving }) => {
    return (
        <div className="space-y-4">
            {addresses.length === 0 && <p className="text-sm text-gray-500">No delivery addresses saved.</p>}
                ...
                <div key={address.id} className={`border p-3 rounded-md flex justify-between items-start ${address.isDefault ? 'border-blue-300 bg-blue-50' : ''}`}>
                    <div>
                        <p className="font-medium">
                            {address.label ? `${address.label}` : `Address ${address.id.substring(0, 5)}`}
                            {address.isDefault && <span className="ml-2 text-xs font-semibold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded">Default</span>}
                        </p>
                        <p className="text-sm text-gray-600">{getAddressString(address)}</p>
                    </div>
                    <div className="flex space-x-1 flex-shrink-0 ml-2">
                        <Button type="button" variant="outline" size="sm" onClick={() => onEdit(address)} disabled={saving}>Edit</Button>
                        {/* Prevent deleting the last address */}
                        <Button type="button" variant="destructive" size="sm" onClick={() => onDelete(address.id)} disabled={saving || addresses.length <= 1}>Delete</Button>
                    </div>
                </div>
            ))}
